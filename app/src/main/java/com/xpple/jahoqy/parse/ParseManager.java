package com.xpple.jahoqy.parse;
...
public class ParseManager {
	...
	public void getContactInfos(List<String> usernames, final EMValueCallBack<List<HuanXinUser>> callback) {
		ParseQuery<ParseObject> pQuery = ParseQuery.getQuery(CONFIG_TABLE_NAME);
		...
		pQuery.findInBackground(new FindCallback<ParseObject>() {
			...
			public void done(List<ParseObject> arg0, ParseException arg1) {
				if (arg0 != null) {
					List<HuanXinUser> mList = new ArrayList<HuanXinUser>();
     ...
     * @param user
     */
    private static void setUserHearder(HuanXinUser user) {
	...
	public void asyncGetCurrentUserInfo(final EMValueCallBack<HuanXinUser> callback){
		final String username = EMChatManager.getInstance().getCurrentUser();
		asyncGetUserInfo(username, new EMValueCallBack<HuanXinUser>() {
				...
				if (error == ParseException.OBJECT_NOT_FOUND) {
					...
					pUser.saveInBackground(new SaveCallback() {
						...
						public void done(ParseException arg0) {
							if(arg0==null){
								callback.onSuccess(new HuanXinUser(username));
							}
						}
					});
				}else{
		...
		pQuery.getFirstInBackground(new GetCallback<ParseObject>() {
			...
			public void done(ParseObject pUser, ParseException e) {
				if(pUser!=null){
					...
					if(callback!=null){
						HuanXinUser user = UserUtils.getUserInfo(username);
						if(user!=null){
							user.setNick(nick);
