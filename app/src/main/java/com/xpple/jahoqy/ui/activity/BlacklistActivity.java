...
public class BlacklistActivity extends Activity {
	...
	private class BlacklistAdapater extends ArrayAdapter<String> {
		...
		public View getView(int position, View convertView, ViewGroup parent) {
			...
			query.addWhereEqualTo("mobilePhoneNumber", getItem(position));
			query.findObjects(getContext(), new FindListener<User>() {
				@Override
				public void onSuccess(List<User> list) {
					if (list != null && list.size() > 0) {
						name.setText(list.get(0).getUsername());
						ImageDownLoader.showNetImage(getContext(), list.get(0).getUserPhoto(), avatar, R.drawable.default_avatar);
					}
				}
