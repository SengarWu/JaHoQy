...
import com.xpple.jahoqy.adapter.UserSeekAdapter;
 ...
 */
public class UserSeekFragment extends BaseFragment {
    private View parentView;
    private ZrcListView listView;
    private Handler handler;//用于接收子线程的信息以刷新主线程
    private UserSeekAdapter adapter;
    ...
    private void setUpView() {
        ...
        adapter = new UserSeekAdapter(getActivity(), items);
    ...
    private void queryUserSeek() {
        BmobQuery<SeekHelp> query=new BmobQuery<>();
        User a=new User();
        a.setObjectId(obid);
        query.addWhereEqualTo("user", a);
        query.setLimit(10);
        query.addQueryKeys("-updatedAt,state,title");
    ...
    private void querymore(int curPage){
        ...
        query.findObjects(getActivity(), new FindListener<SeekHelp>() {
            @Override
            public void onSuccess(List<SeekHelp> object) {
