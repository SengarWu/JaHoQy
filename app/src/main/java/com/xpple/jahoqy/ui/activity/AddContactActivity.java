...
public class AddContactActivity extends BaseActivity {
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		View view = (View) findViewById(R.id.edit_note);
	...
	public void searchContact(View v) {
		...
		if (getString(R.string.button_search).equals(saveText)) {
			...
			query.addWhereEqualTo("mobilePhoneNumber", toAddUsername);
			query.findObjects(mContext, new FindListener<User>() {
				@Override
				public void onSuccess(List<User> list) {
					if (list != null && list.size() > 0) {
						nameText.setText(list.get(0).getUsername());
						ImageDownLoader.showNetImage(mContext, list.get(0).getUserPhoto(), avatar, R.drawable.default_avatar);
					}
				}
	...
	public void addContact(View view){
		if(CustomApplication.getInstance().getUserName().equals(nameText.getText().toString())){
